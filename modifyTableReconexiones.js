// Script para agregar un campo autoincremental y clave primaria a la tabla 'reconexiones' en Railway
// Uso: node modifyTableReconexiones.js
...
async function modifyTable() {
  const tableName = 'reconexiones';
  const newColumn = 'id_reconexion';

  const connection = await mysql.createConnection(dbConfig);

  try {
    // 1. Modifica id_cuenta para que no sea AUTO_INCREMENT ni clave primaria
    try {
